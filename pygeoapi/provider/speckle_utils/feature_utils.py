...
def initialize_features(self: "SpeckleProvider", all_coords, all_coord_counts, data, context_list,
                        comments: Dict) -> None:
    ...
    if len(data["features"]) == 0 and len(data["comments"]) == 0:
        raise ValueError(
            f"No supported features of type '{self.requested_data_type}' found. Make sure correct type is requested by adding a URL parameter (e.g. '&dataType=points').")
...
def assign_comment_data(comments, properties):
    ...
    for item in comments:
        ...
        '''
        try:
            formatted_time = datetime.strptime(item["date"].replace("T", " ").replace("Z", "").split(".")[0],
                                               '%Y-%m-%d %H:%M:%S')
        except:
            formatted_time = item["date"]

        properties["messages"].append(
            f"Author: {item['author']}, created: {formatted_time}, text: {item['text']}, attachments: {[img for img in item['attachments']]}")
...
def create_features(self: "SpeckleProvider", context_list: List["TraversalContext"], comments: Dict,
                    data: Dict) -> None:
